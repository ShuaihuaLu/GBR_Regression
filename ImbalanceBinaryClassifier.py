...
def data_load(filename, Sheet):
    ...
    for i in range(1, nrows):
        ...
        result.append(rows)
    ID = [int(x[0]) for x in result]  # ID
    formula = [x[1] for x in result]  # 化学式
    prototype = [x[2] for x in result]  # 初始结构名称
    features = np.array([x[3: -1] for x in result])  # features
    label = np.array([x[-1] for x in result])  # 标签列
    features_name = np.array(table.row_values(0))[3: -1]
...
def GBClassifier(trainX, testX, trainY, testY, max_evals, Loop_Step, screen_step):
    ...
    def function(argsDict):
        ...
        eec = EasyEnsembleClassifier(n_estimators=5, base_estimator=clf, random_state=42)
    ...
    best_model = EasyEnsembleClassifier(n_estimators=5, base_estimator=based_best_model, random_state=42)
...
workbook = "Magnetic-20+SISSO-train&test-loop-4.xlsx"  # Input data excel
sheet = str("NM-M+SISSO")  # Input data excel sheet
n_fixed_features = 92  # the number of fixed features
...
LoopStepMax = 1  # Maximum  number of iteration loop
RandomSeed = 42  # Random seed for data split
TestSetRatio = 0.2  # Test set ratio
number_sample = 3786  # Number of train&test data
